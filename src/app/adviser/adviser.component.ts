...
export class AdviserComponent implements OnInit {
  ...
  generateAdvice(data: any[]): void {
    ...
    this.advice.push(
      ...weekdayLightingAdvice,
      ...weekendLightingAdvice,
      ...weekdayComputerAdvice,
      ...weekendComputerAdvice
    );

    if (this.advice.length === 0) {
      this.advice.push({
        message: "Your recent energy usage is optimal. Keep up the good work!",
        timestamp: "",
      });
    }
  }
  ...
  checkLightingUsage(data: any[], isWeekend: boolean): AdviceMessage[] {
    ...
    data.forEach((day) => {
      ...
      for (let i = 18; i < 24; i++) {
        // Check from 6 PM to midnight
        if (hours[i] > 0) {
          const timestamp = new Date(day.date).toLocaleString();
          const message = `Consider reducing lighting usage during evening hours ${
            isWeekend ? "on weekends" : "on weekdays"
          } to save energy. Detected on ${timestamp}.`;
  ...
  checkComputerUsage(data: any[], isWeekend: boolean): AdviceMessage[] {
    ...
    data.forEach((day) => {
      ...
      if (hours.some((hour: number) => hour > 0)) {
        const timestamp = new Date(day.date).toLocaleString();
        const message = `Avoid prolonged computer usage ${
          isWeekend ? "during weekends" : "during weekdays"
        } to conserve energy. Detected on ${timestamp}.`;
        adviceMessages.push({ message, timestamp });
      }
    });
